...
MIGRATION="20260924063000_growth_landing_attribution.sql"
...
log "Applying growth landing-attribution migration"
docker run --rm \
  -e DIRECT_URL="$DB_URL" \
...
log "Verifying growth attribution function"
FUNCTION_STATE="$(
  docker run --rm -i -e DIRECT_URL="$DB_URL" postgres:16-alpine sh -ec 'psql "$DIRECT_URL" -v ON_ERROR_STOP=1 -At' <<'SQL'
select case
  when pg_get_functiondef('public.track_payment_intent_growth_event()'::regprocedure) like '%landingPath%'
  then 'growth_attribution_ok'
  else 'growth_attribution_invalid'
end;
SQL
)"
[ "$FUNCTION_STATE" = "growth_attribution_ok" ] || fail "Growth attribution function is not current"
unset DB_URL
...
echo
echo "============================================================"
echo "MyPets Growth v18 deployed and verified."
