...
log "Applying cumulative MyPets Platform v2 migrations"
for migration in \
  20260905050500_identity_roles.sql \
  20260905054000_growth_funnels.sql \
  20260905060000_causes_sponsorships.sql
do
  echo " -> $migration"
  docker run --rm \
    -e DIRECT_URL="$DB_URL" \
...
if ! grep -q '^DISCOVERY_INGEST_TOKEN=..' "$ENV_FILE"; then
  log "Generating private discovery ingest token"
  set_env "DISCOVERY_INGEST_TOKEN" "$(openssl rand -hex 32)"
fi

log "Updating production flags"
set_env "PUBLIC_DEMO_CONTENT" "false"
set_env "APP_VERSION" "0.6.0"
...
log "Checking Platform v2 public surfaces"
...
echo
curl -fsS 'https://api.mypets.lat/v1/causes?limit=1'
echo

echo
echo "============================================================"
echo "MyPets Platform v2 deployed."
echo "Identity + Growth + Causes + Sponsorships + Social/Discovery"
echo "are active. Discovery ingest token stays server-side only."
