-- Preserve paid-media identifiers through server-trusted payment conversion events.
-- These identifiers are stored in payment intent metadata and copied only when
-- a real payment state transition emits DONATION_STARTED / DONATION_COMPLETED.
...
begin
  ...
  insert into public.growth_events (
    ...
    jsonb_strip_nulls(jsonb_build_object(
      ...
      'term', new.metadata->>'term',
      'utmId', new.metadata->>'utmId',
      'sourcePlatform', new.metadata->>'sourcePlatform',
      'gclid', new.metadata->>'gclid',
      'gbraid', new.metadata->>'gbraid',
      'wbraid', new.metadata->>'wbraid',
      'fbclid', new.metadata->>'fbclid',
      'msclkid', new.metadata->>'msclkid',
      'ttclid', new.metadata->>'ttclid',
      'rewardKeys', new.metadata->'rewardKeys',
      'campaignBaseAmountCents', new.metadata->'campaignBaseAmountCents',
      'extraSupportCents', new.metadata->'extraSupportCents',
      'foodKg', new.metadata->'foodKg'
    ))
  );
...
comment on function public.track_payment_intent_growth_event() is
  'Emits server-trusted payment funnel events with acquisition landing, paid-media identifiers and campaign unit metadata.';
